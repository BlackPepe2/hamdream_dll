...
int CMOTDABEnc::GetPicSegmPerc(void)
{
	if (iSegmCnt == 0) return 0;
	if (iTotSegm == 0) return 100;
	return (100 * iSegmCnt) / iTotSegm;
}
			...
			(biTransportIDFlag == 1))
		{
			...
			if (iDataGroupType == 3)
			{
				...
				if (iSegmentNum == 0)
				{
					...
					{
#ifdef debfil
						fprintf(outfil,"\nHead Reset TransID new %d old %d",iTransportID,MOTObjectRaw.iTransportID);
#endif
						// store in pool
...
#ifdef debfil
					fprintf(outfil,"\nHead seg %d  siz %d  id %d",
						iSegmentNum,iSegmentSize,iTransportID);
#endif
				}
				else
				{
			...
			else if (iDataGroupType == 4)
			{
				...
				/* Body ----------------------------------------------------- */
				{
					...
					if (MOTObjectRaw.iTransportID == iTransportID)
					{
						...
						{
#ifdef debfil
							fprintf(outfil,"\nSegsize Init to %d",iSegmentSize);
#endif
						...
						{
#ifdef debfil
							fprintf(outfil,"\nPoolremove wrong segsize %d  oldsiz %d",
								iSegmentSize,MOTObjectRaw.iSegmentSize);
#endif
...
#ifdef debfil
						fprintf(outfil,"\nBody seg %d  siz %d  tot %d  act %d",
							iSegmentNum,iSegmentSize,MOTObjectRaw.BodyRx.iTotSegments,
							MOTObjectRaw.BodyRx.iDataSegNum);
#endif
					}
					else
					{
						...
						PicPool.storeinpool(MOTObjectRaw);
#ifdef debfil
						fprintf(outfil,"\nBody Reset TransID new %d old %d",iTransportID,MOTObjectRaw.iTransportID);
#endif	
						PicPool.getfrompool(iTransportID,MOTObjectRaw);
...
#ifdef debfil
					fprintf(outfil,"\nBody Set Tot %d",MOTObjectRaw.BodyRx.iTotSegments);
#endif
				}
...
#ifdef debfil
							fprintf(outfil,"\nBody OK Seg %d",MOTObjectRaw.Body.iDataSegNum);
#endif
						}
					}
				}
...
#ifdef debfil
					fprintf(outfil,"\nDecode %d Seg",MOTObjectRaw.BodyRx.iTotSegments);
#endif
...
_BOOLEAN	CMOTDABDec::GetActBSR(int * iNumSeg, string * bsr_name, int * iHash)
{
	FILE * bsr;
...
#ifdef debfil
	fprintf(outfil,"\nWriting Seg %d Size %d",iSegNum,iSegmentSize);
#endif
}
...
#ifdef debfil
	fprintf(outfil,"\nWriting seg %d len %d",iSegNum,iSegmentSize);
	fprintf(outfil," newnumele %d oldnumele %d",iNewEnlSize,iOldEnlSize);
	fprintf(outfil," NumSegments %d ",iDataSegNum);
#endif
